...
void get_defconfig_path(char *out, size_t bufsize, char *argv0)
{
	...
	struct stat info;
#ifdef WIN32
	snprintf(out, bufsize, "%s\\XpsMiner\\XpsMiner.conf\0", getenv("APPDATA"));
#else
	snprintf(out, bufsize, "%s\\.XpsMiner\\XpsMiner.conf", getenv("HOME"));
#endif
	if (dir && stat(out, &info) != 0) {
		// binary folder if not present in user folder
		snprintf(out, bufsize, "%s%sXpsMiner.conf%s", dir, sep, "");
	}
	if (stat(out, &info) != 0) {
		out[0] = '\0';
		return;
...
bool stratum_subscribe(struct stratum_ctx *sctx)
{
	...
	char *s, *sret = NULL;
	const char *sid;
	json_t *val = NULL, *res_val, *err_val,*params;
	json_error_t err;
	bool ret = false, retry = false;
	...
	if (sctx->solo) {
		...
		strcpy(sctx->job.job_id, "solo");

		hex2bin(sctx->job.prevhash, prevhash, 20);
		...
		sctx->xnonce1 = (uchar*)calloc(1, sctx->xnonce2_size);
...
bool stratum_authorize(struct stratum_ctx *sctx, const char *user, const char *pass)
{
	json_t *val = NULL, *res_val, *err_val, *type_val;
	char *s, *sret;
	json_error_t err;
	bool ret = false;
	...
	type_val = json_object_get(val, "type");

	if (type_val){
		if (strcmp(json_string_value(type_val), "solo") == 0) {
			sctx->solo = true;
			ret = true;
			goto out;
		}
...
static bool stratum_pong(struct stratum_ctx *sctx, json_t *id)
{
	...
	sprintf(buf, "{\"method\":\"pong\",\"params\":[]}");
...
bool stratum_handle_method(struct stratum_ctx *sctx, const char *s)
{
	...
	if (!strcasecmp(method, "client.get_algo")) { // XpsMiner only yet!
	...
	if (!strcasecmp(method, "client.get_stats")) { // XpsMiner/yiimp only yet!
